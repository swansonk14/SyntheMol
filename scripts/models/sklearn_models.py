...
from synthemol.constants import (
    DATASET_TYPES,
    SKLEARN_MODEL_NAME_TYPES,
    SKLEARN_MODEL_TYPES,
)


def sklearn_save(model: SKLEARN_MODEL_TYPES, save_path: Path) -> None:
...
def sklearn_build_model(
    model_type: SKLEARN_MODEL_NAME_TYPES, dataset_type: DATASET_TYPES
) -> SKLEARN_MODEL_TYPES:
...
def sklearn_train(
    ...
    """
    # Build model
    model = sklearn_build_model(model_type=model_type, dataset_type=dataset_type)

    # Train model
    model.fit(fingerprints, properties)

    # Save model
    sklearn_save(model=model, save_path=save_path)
