...
def plot_mcts_over_time(
    ...
    for rollout_num in range(0, num_rollouts, increment):
        rollout_data = data[
            (data[ROLLOUT_COL] > rollout_num)
            & (data[ROLLOUT_COL] <= rollout_num + increment)
        ]
        rollout_bins.append((rollout_num, rollout_num + increment))
        rollout_bin_scores.append(rollout_data[SCORE_COL])

    rollout_bin_labels = [
        f"{rollout_bin_start + 1:,} - {rollout_bin_end:,}"
        for rollout_bin_start, rollout_bin_end in rollout_bins
    ]
    ...
    if plot_type == "histogram":
        ...
        for (rollout_bin_start, rollout_bin_end), rollout_bin_score in zip(
            rollout_bins, rollout_bin_scores
        ):
            plt.hist(
                rollout_bin_scores,
