...
def compute_curve(
    activities: pd.Series, predictions: pd.Series, curve_type: Literal["ROC", "PRC"]
...
def plot_auc(
    ...
    for i, test_path in enumerate(data_dir.rglob("*_test_preds.csv")):
        ...
        x_values, y_values, auc_score = compute_curve(
            data[activity_column], data[prediction_column], curve_type
        )
    ...
    save_name = (
        save_dir / f'{model_name.lower().replace(" ", "_")}_{curve_type.lower()}'
    )
