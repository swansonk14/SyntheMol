...
def plot_scores(
    scores: list[float], save_dir: Path, score_name: str = SCORE_COL
) -> None:
...
def plot_reactions_numbers(num_reactions: list[int], save_dir: Path) -> None:
...
def plot_building_block_usage(data: pd.DataFrame, save_dir: Path) -> None:
    ...
    building_block_counter = Counter(
        ...
        for building_block in {
            int(building_block) for building_block in building_block_row.dropna()
        }
        if building_block != -1
    )
    ...
    fig_data = pd.DataFrame(
        {"building_block": building_blocks, "count": building_block_counts}
    )
...
def plot_generated_molecule_analysis(
    ...
    # Score distribution
    plot_scores(scores=data[score_column], save_dir=save_dir)
    ...
    # Number of reactions
    plot_reactions_numbers(num_reactions=data["num_reactions"], save_dir=save_dir)

    # Usage of reactions
    plot_reaction_usage(data=data, save_dir=save_dir)

    # Usage of building blocks
    plot_building_block_usage(data=data, save_dir=save_dir)
