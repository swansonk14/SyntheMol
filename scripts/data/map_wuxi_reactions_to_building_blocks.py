...
def map_wuxi_reactions_to_building_blocks(
    ...
    reaction_to_reactant_to_building_blocks: dict[int, dict[int, set[str]]] = {
        reaction.id: {
            reactant_index: sorted(
                {
                    building_block
                    for building_block in data[
                        data[subset_column].isin(reactant.subsets)
                    ][smiles_column]
    ...
    ) in reaction_to_reactant_to_building_blocks.items():
        for reactant, building_blocks in reactant_to_building_blocks.items():
            if len(building_blocks) == 0:
                raise ValueError(
                    f"No building blocks found for reaction {reaction} reactant {reactant}"
                )
