...
def merge_wuxi_building_blocks(
    ...
    phase_2 = pd.DataFrame(
        {
            ...
            subset_column: [
                f"phase_2_{core}" for core in phase_2_cores[WUXI_CORES_CORE_COL]
            ],
        }
    )
    phase_3 = pd.DataFrame(
        {
            ...
            subset_column: [
                f"phase_3_{core}" for core in phase_3_cores[WUXI_CORES_CORE_COL]
            ],
        }
    )
    ...
    data[id_column] = data[[wuxi_id_column, subset_column]].apply(
        lambda row: f"{row[wuxi_id_column]}_{row[subset_column]}", axis=1
    )

    # Print dataset statistics
    print(f"Number of unique building block IDs = {data[id_column].nunique():,}")
    print(
        f"Number of unique building block WuXi IDs = {data[wuxi_id_column].nunique():,}"
    )
    print(
        f"Number of unique building block subsets = {data[subset_column].nunique():,}"
    )
